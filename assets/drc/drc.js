var WRs,
    ...
    PHANTASMAGORIA = "#phantasmagoriaTable",
    BOMBS = "#bombs",
    SCORE = "#score",
    ...
    MISSES = "#misses",
    IMP_NOT = "#impNot",
    IMP_NOT_TEXT = "#impNotText",
    CHALLENGE = "#challenge",
    NO_EXTRA = "<option>Easy</option>\n<option>Normal</option>\n<option>Hard</option>\n<option>L" +
            "unatic</option>",
    SHOTTYPE_MULTIPLIERS = "#shottypeMultipliersTable",
    ...
    SHOTTYPE_LABEL = "#shottypeLabel",
    MISSES_INPUT = "<label id='missesLabel' for='misses'>Misses</label><input id='misses' type='numb" +
            "er' value=0 min=0 max=100>",
    SCORING_TABLE = "#scoringTable",
    SURV_TABLE = "#survivalTable",
    CLEARED = "#cleared",
    SCENE = "#scene",
    POFV_FORMULA = "#pofvFormula",
    ROUTE = "#route",
    SCORE_OPTIONS = "<label id='scoreLabel' for='score'>Score</label><input id='score' type='text'>",
    DS_TABLE = "#dsTable",
    MOF_TABLE = "#mofTable",
    RELEASES = "#releases",
    SEASON = "#season",
    ...
    HSIFS = "#hsifs",
    WBAWC = "#wbawc",
    MISSES_LABEL = "#missesLabel",
    BOMBS_LABEL = "#bombsLabel",
    NB_LABEL = "#nbLabel",
    DIFFICULTY_LABEL = "#difficultyLabel",
    RELEASES_LABEL = "#releasesLabel",
    SURVIVAL = "#survival",
    SCORING = "#scoring",
    IS_LABEL = "#isLabel",
    LS_LABEL = "#lsLabel",
    ...
    FINALA = "#finala",
    FINALB = "#finalb",
    SCORING_COLUMN = "<th id='game0'>Game</th><th id='maxPoints0'>Max points</th><th id='exp0'>Exponen" +
            "t</th>",
    BB_LABEL = "#bbLabel",
    SURV_COLUMN = "<th id='game2'>Game</th><th id='maxPoints1'>Max points</th><th id='base0'>Base</" +
            "th><th id='lostLife'>Lost life (n)</th><th id='firstBomb'>First bomb (n)</th><th" +
            " id='further'>Further bombs (n)</th>",
    FIRST_BOMB = "#firstBomb",
    FURTHER = "#further",
    LOST_LIFE = "#lostLife",
    POFV_SURV = "#phantasmagoria",
    POFV_SURV_DESC = "#phantasmagoriaDesc",
    MOFAITH = "#mountainOfFaith",
    MOFAITH_DESC = "#mountainOfFaithDesc",
    SHOT_MULT = "#shottypeMultipliers",
    RULES_TEXT = "#rulesText",
    SHOT_MULT_DESC = "#shotMultDesc",
    BASE_POINTS = "#basePoints",
    MULTIPLIEDSHOTTYPE = "#multipliedShottype",
    ...
    RUBRICS_TEXT = "#rubricsText",
    ACK_TEXT = "#ackText",
    DRC_INTRO = "#drcIntro",
    CATEGORY = "#category",
    BACK_TO_TOP = "#backToTop",
    SCORE_FORMULA = "#scoreFormula",
    DRC_INTRO_PTS = "#drcIntroPts",
    DRC_SCORES = "#drcScores",
    RUBRICS_EXPL = "#rubricsExpl",
    SCORING_NOTES = "#scoringNotes",
    SURV_NOTES = "#survivalNotes",
    NEW_WR = "#newWR",
    MOF_SEPARATE = "#mofSeparate",
    MAINGAME = "#maingame",
    PHANTASMAGORIA_SEPARATE = "#phantasmagoriaSeparate",
    THRESHOLD = "#threshold",
    INCREMENTS = "#increments",
    IN_LS = "#inLS",
    HSIFS_RELEASES = "#hsifsReleases",
    CALCULATE = "#calculate",
    MAX_LIVES = "#maxLives",
    SURV_FORMULA = "#survFormula",
    CREDIT = "#credit",
    JP_TL_CREDIT = "#jptlcredit",
    CN_TL_CREDIT = "#cntlcredit",
    COUNTDOWN = "#countdown",
    NO_CHARGE_LABEL = "#ncLabel",
    RULE1 = "#rule1",
    RULE2 = "#rule2",
    RULE3 = "#rule3",
    DS_SEPARATE = "#dsSeparate",
    DOUBLE_SPOILER = "#doubleSpoiler",
    DOUBLE_SPOILER_DESC = "#doubleSpoilerDesc",
    DS_TABLE = "#dsTable",
    DS = "#ds",
    GAME = "#game",
    BB = "#bb",
    ...
    FWR = "#fictionalWR",
    WR_DEF = "#WRdefinition",
    FWR_TITLE = "#fictionalWRtitle",
    FWR_DESC = "#fictionalWRdesc",
    FWR_TABLE = "#fictionalWRtable",
    WR_DEF_TITLE = "#WRdefinitionTitle",
    WR_DEF_DESC = "#WRdefinitionDesc",
    WR_DEF_TABLE = "#WRdefinitionTable",
    SURV_BUTTON = "#survivalButton",
...
function generateText(firstTime) {
    var numberOfShottypes = $(SHOTTYPE + " option").length,
        i;
    if (language == "English") {
        $(HRTP).html("HRtP");
        $(SOEW).html("SoEW");
        ...
        $(RULE2).html("Replays are required for Windows game submissions, while for PC-98 a video is ac" +
                "cepted.");
        $(RULE3).html("All runs must be played using at most default lives and bombs.");
        ...
        if ($(SURV_BUTTON).val() == "Show Survival Rubrics" || $(SURV_BUTTON).val() == "クリア重視のルーブリックを見せて" || $(SURV_BUTTON).val() == "显示生存计算公式") {
            $(SURV_BUTTON).val("Show Survival Rubrics")
        } else {
            $(SURV_BUTTON).val("Hide Survival Rubrics")
        }
        if ($(SCORE_BUTTON).val() == "Show Scoring Rubrics" || $(SCORE_BUTTON).val() == "稼ぎのルーブリックを見せて" || $(SCORE_BUTTON).val() == "显示打分计算公式") {
            $(SCORE_BUTTON).val("Show Scoring Rubrics")
        } else {
            $(SCORE_BUTTON).val("Hide Scoring Rubrics")
        }
        $(CALCULATE).val("Calculate");
        $(SCORING_NOTES).html("Scoring Notes");
        $(SURV_NOTES).html("Survival Notes");
        $(WR_PAGE).html("The WRs are taken from <a href='/wr'>this page</a>.");
        $(NEW_WR).html("If you achieve a new World Record, your points are equal to the max points; othe" +
                "rwise, the formula applies.");
        $(FWR).html("Some categories use a fictional WR. <a href='#fictionalWRtitle'>Click here</a> f" +
                "or the list of them.");
        $(WR_DEF).html("Some categories always use the WR of a specific shottype. <a href='#WRdefinition" +
                "Title'>Click here</a> for the list of them.");
        $(MOF_SEPARATE).html("MoF uses a separate system. <a href='#mountainOfFaith'>Click here</a> for said s" +
                "ystem.");
        $(DS_SEPARATE).html("DS uses a separate system. <a href='#doubleSpoiler'>Click here</a> for said syst" +
                "em.");
        $(MAINGAME).html("For a main game clear, a shottype multiplier is applied to your DRC points, the " +
                "result of which is again rounded. <a href='#shottypeMultipliers'>Click here</a> " +
                "for the list of them.");
        $(PHANTASMAGORIA_SEPARATE).html("The Phantasmagorias use a separate system. <a href='#phantasmagoria'>Click here<" +
                "/a> for said system.");
        $(IN_LS).html("For IN, you obtain 2 (1 on Easy) additional points for each captured Last Spell," +
                " with the exception of Imperishable Shooting, which yields 5 points.");
        $(HSIFS_RELEASES).html("For HSiFS, the first release adds 2 to <em>n</em>, and further releases add 0.5," +
                " 0.4, 0.3, 0.2, 0.1 to <em>n</em>.");
        $(FWR_TITLE).html("Fictional WRs");
        $(FWR_DESC).html("The categories in the table below use a fictional WR instead of the real one.");
        $(WR_DEF_TITLE).html("'WR' definition");
        $(WR_DEF_DESC).html("The categories in the table below always use the WR of a specific shottype.");
        $(MOFAITH).html("MoF Scoring");
        $(MOFAITH_DESC).html("For each difficulty and shottype there are six thresholds, at which you will hav" +
                "e set numbers of points respectively. Then, increments are done, dependent on ho" +
                "w much higher than the threshold your score is. The maximum is 375 on Easy and 5" +
                "00 on Lunatic.");
        $(DOUBLE_SPOILER).html("DS Scoring");
        $(DOUBLE_SPOILER_DESC).html("For each scene there are three thresholds, at which you will have set numbers of" +
                " points points respectively. Then, increments are done, dependent on how much hi" +
                "gher than the threshold your score is.");
        $(POFV_SURV).html("PoDD & PoFV Survival");
        $(POFV_SURV_DESC).html("In the below formula, MaxLives is equal to 5 for PoDD, 7 for PoFV main game and " +
                "8 for PoFV Extra. NoBombBonus is a difficulty-specific No Bomb bonus for PoDD an" +
                "d a No Charge Attacks bonus for PoFV. RoundsLost is equal to how many rounds the" +
                " player lost.");
        $(SHOT_MULT).html("Shottype Multipliers");
        $(SHOT_MULT_DESC).html("These are applied to the result of the survival formula for a main game run only" +
                "; they do <em>not</em> apply for Extra, nor do they apply for HSiFS runs that us" +
                "e releases. For all shots not listed here, the shottype multipliers are equal to" +
                " 1.");
        ...
        for (i = 0; i < numberOfShottypes; i += 1) {
        ...
        $(DRC_INTRO).html("The <b>Dodging Rain Competition (DRC)</b> is a Touhou game competition that was " +
                "invented by <a href='https://twitter.com/VincentZeem'>ZM</a> and is held on <a h" +
                "ref='https://discord.gg/Ucae3Uf'>the official DRC Discord</a>. Two teams go up a" +
                "gainst each other in several different categories. Each player posts an arbitrar" +
                "ily long list of categories, ordered by preference, which can be either survival" +
                " or scoring of any Touhou shooting game and any difficulty. They will be matched" +
                " up against a player from the other team, in a category that both players had on" +
                " their list. The teams and categories are determined by the DRC management team." +
                " Players are given two weeks to sign up for the competition, and once it starts," +
                " two weeks to submit a replay, which will be awarded points dependent on the rub" +
                "rics. Runs done outside those two weeks are invalid. Players can submit an unlim" +
                "ited number of replays; the replay that is worth the most DRC points will count.");
        $(DRC_INTRO_PTS).html("If you want to know how many DRC points a run is worth, the points for a given r" +
                "un can be determined using the calculator below.");
        $(DRC_SCORES).html("Scores can only contain digits, commas, dots and spaces. Survival runs are assum" +
                "ed to have cleared, scoring runs not.");
        $(RUBRICS_EXPL).html("The rubrics are the formulas and fixed values used to calculate the number of DR" +
                "C points for a run. If you are curious about how your points are being determine" +
                "d, click the button below to expand.");
        $(CREDIT).html("The background image was drawn by <a href='https://www.pixiv.net/member.php?id=1" +
                "61300'>ウータン</a>.");
        $(JP_TL_CREDIT).html("The Japanese translation was done by <a href='https://twitter.com/7bitm'>7bitm</" +
                "a> and <a href='https://twitter.com/toho_yumiya'>Yu-miya</a>.");
        $(CN_TL_CREDIT).html("The Simplified Chinese translation was done by <a href='https://twitter.com/Izay" +
                "oiMeirin'>Cero</a>, <a href='https://twitter.com/CrestedPeak9'>CrestedPeak9</a> " +
                "and <a href='https://twitter.com/Cerasis_th'>Cerasis</a>.")
    } else if (language == "Japanese") {
        ...
        $(RULE2).html("Windows版作品の登録にはリプレイが必要です。PC-98版作品においては録画ファイルにて受け付け可能です。");
        $(RULE3).html("プレイに於いては、初期残機ボムにて参加すること。ただし初期残機ボムを操作することが出来る作品においては減らすことのみ可能とする。なおペナルティーはないものとする" +
                "。");
        $(NB_LABEL).html("ノーボム");
        ...
        if ($(SURV_BUTTON).val() == "Show Survival Rubrics" || $(SURV_BUTTON).val() == "显示生存计算公式") {
            $(SURV_BUTTON).val(translate("Show Survival Rubrics"))
        } else {
            $(SURV_BUTTON).val(translate("Hide Survival Rubrics"))
        }
        if ($(SCORE_BUTTON).val() == "Show Scoring Rubrics" || $(SCORE_BUTTON).val() == "显示打分计算公式") {
            $(SCORE_BUTTON).val(translate("Show Scoring Rubrics"))
        } else {
            $(SCORE_BUTTON).val(translate("Hide Scoring Rubrics"))
        }
        $(CALCULATE).val("計算する");
        $(SCORING_NOTES).html("稼ぎのノート");
        $(SURV_NOTES).html("クリア重視のノート");
        $(WR_PAGE).html("WRのリストは<a href='/wr'>こちら</a>です。");
        $(NEW_WR).html("もし新世界記録を達成すれば、あなたのポイントは最高点になります。そうでなければ式を適用します。");
        $(FWR).html("いくつかのカテゴリには、仮のWRを設定してあります。仮のWRのリストは<a href='#fictionalWRtitle'>ここをクリック</a>。");
        $(WR_DEF).html("幾つかのカテゴリには、特定のショットタイプによるWRが設定されています。WRの定義のリストは<a href='#WRdefinitionTitle'>ここをクリ" +
                "ック</a>。");
        ...
        $(MAINGAME).html("本編クリアではキャラ倍率がDRCポイントに掛けられます。その結果は四捨五入されます。キャラ倍率のリストは<a href='#shottypeMultiplier" +
                "s'>ここをクリック</a>。");
        ...
        $(FWR_TITLE).html("仮のWR");
        $(FWR_DESC).html("下記のものは、本来のWRの代わりに仮のWRを設定をしたものの表です。");
        $(WR_DEF_TITLE).html("WRの定義");
        $(WR_DEF_DESC).html("表には、WRとして採用したショットタイプを紹介しています。");
        $(MOFAITH).html("東方風神録の稼ぎ");
        $(MOFAITH_DESC).html("各難易度各機体で６つの閾値があり、それぞれで点数を設定しています。その後、あなたのスコアがどれだけ閾値より高いかに基づき増分します。Easyの最大点は375です" +
                "。Lunaticの最大点は500です。");
        ...
        $(POFV_SURV_DESC).html("下式において、最大残機を夢時空で５、花映塚本編で７、花映塚エキストラでは８です。ノーボムボーナスは難易度ごとに変わる、夢時空でのボムの不使用ボーナスと花映塚の（" +
                "Ｃ１含む）チャージ攻撃の不使用ボーナスです。");
        $(SHOT_MULT).html("キャラ倍率");
        $(SHOT_MULT_DESC).html("これらは本編のクリア重視プレイの結果にのみ適用されます。Extraでは適用されません。解放を使用した天空璋のプレイにも適用されません。ここに載っていない機体のキ" +
                "ャラ倍率は１となります。");
        ...
        $(DRC_INTRO).html("<b>Dodging Rain Competition(DRC)</b>は<a href='https://twitter.com/VincentZeem'>Z" +
                "M</a>により考案された<a href='https://discord.gg/Ucae3Uf'>DRC Discord</a>で開かれる東方projectの" +
                "定期大会です。２つのチームが幾つかのカテゴリーで競争します。各プレイヤーは希望順に並べたカテゴリーのリストを作ります。カテゴリーは東方STGゲームの任意の難易度" +
                "での「クリア重視」と「スコアアタック（稼ぎ）」のどちらかを選ぶことができます。相手チームのプレイヤーと、共通してリストされていた１つのカテゴリーで、マッチングさ" +
                "れます。このチームとカテゴリーはDRC運営陣によって決められます。プレイヤーには大会登録のために２週間が与えられます。そして大会が始まり、リプレイ提出のための２" +
                "週間が与えられます。その後ルーブリックに基づいてポイントが授与されます。この２週間以外でのプレイは無効です。プレイヤーは無制限にリプレイを登録することが出来ます" +
                "が、DRCポイントが最高であるリプレイのみが採用されます。");
        ...
        $(CN_TL_CREDIT).html("<a href='https://twitter.com/IzayoiMeirin'>Cero</a>と<a href='https://twitter.com" +
                "/CrestedPeak9'>CrestedPeak9</a>と<a href='https://twitter.com/Cerasis_th'>Cerasis" +
                "</a>によって中国語に翻訳されました。")
    } else {
        $(HRTP).html(translateGameName("HRtP"));
        $(SOEW).html(translateGameName("SoEW"));
        ...
        $(RULE2).html("windows平台游戏需要提交rep；PC98平台游戏可提交视频。");
        $(RULE3).html("所有游戏必须在最大默认残机数、bomb数下进行。");
        $(NB_LABEL).html("禁雷");
        ...
        if ($(SURV_BUTTON).val() == "Show Survival Rubrics" || $(SURV_BUTTON).val() == "クリア重視のルーブリックを見せて") {
            $(SURV_BUTTON).val(translate("Show Survival Rubrics"))
        } else {
            $(SURV_BUTTON).val(translate("Hide Survival Rubrics"))
        }
        if ($(SCORE_BUTTON).val() == "Show Scoring Rubrics" || $(SCORE_BUTTON).val() == "稼ぎのルーブリックを見せて") {
            $(SCORE_BUTTON).val(translate("Show Scoring Rubrics"))
        } else {
            $(SCORE_BUTTON).val(translate("Hide Scoring Rubrics"))
        }
        ...
        $(WR_PAGE).html("世界纪录参考自<a href='/wr'>此网页</a>。");
        $(NEW_WR).html("如果获得新世界纪录，你的分数即为最大值。否则，则按公式计算。");
        $(FWR).html("某些项目将参考一个虚构的世界纪录。<a href='#fictionalWRtitle'>单击此处</a>查看列表。");
        $(WR_DEF).html("某些项目将一直参考具体某一机体。<a href='#WRdefinitionTitle'>单击此处</a>查看列表。");
        ...
        $(FWR_TITLE).html("虚构WR");
        $(FWR_DESC).html("下列表中的项目将参考以下虚构的世界纪录。");
        $(WR_DEF_TITLE).html("WR的定义");
        $(WR_DEF_DESC).html("以下表中的项目将独立于其他机体来计算。");
        ...
        $(DRC_INTRO).html("<b>Dodging Rain Competition(DRC)</b>是由<a href='https://twitter.com/VincentZeem'>" +
                "ZM</a>发起的东方比赛，举办地点：<a href='https://discord.gg/Ucae3Uf'>DRC Discord</a>。两队进行不同项目" +
                "的比赛。每位选手报名时写下想打的任意项目，根据偏好排序，可以是避弹向的生存，也可以是打分，任意作品、任意难度均可。列出的项目会与另一队进行匹配，相同项目的选手即" +
                "匹配成功。分组与项目由DRC主办方决定。选手将会有二周时间报名。一旦开赛，有两周时间提交参赛录像，录像所获得分根据计算公式而定。未在两周内提交则视为无效。玩家提" +
                "交的rep个数无限制，最终DRC得分将选取最高分录入。");
        ...
        $(CREDIT).html("The background image was drawn by <a href='https://www.pixiv.net/member.php?id=1" +
                "61300'>ウータン</a>.");
        $(JP_TL_CREDIT).html("本页面由<a href='https://twitter.com/7bitm'>7bitm</a>，<a href='https://twitter.com/t" +
                "oho_yumiya'>ゆーみや</a>日语翻译。");
        $(CN_TL_CREDIT).html("本页面由<a href='https://twitter.com/IzayoiMeirin'>Cero</a>，<a href='https://twitter" +
                ".com/CrestedPeak9'>CrestedPeak9</a>，<a href='https://twitter.com/Cerasis_th'>Cer" +
                "asis</a>中文翻译。")
    }
}
...
function checkShottypes(alwaysChange) {
    ...
    shottypes = (game == "DS"
        ? ["Aya", "Hatate"]
        : Object.keys(WRs[game][difficulty]));
    if (game == "HSiFS") {
...
function drcPoints() {
    ...
    if (challenge == "Survival") {
        ...
        if (game == "HSiFS" && Number($(RELEASES).val()) === 0) {
            season = $(SEASON).val();
            shottypeMultiplier = (Rubrics.SURV[game].multiplier[shottype + season]
                ? Rubrics.SURV[game].multiplier[shottype + season]
                : 1)
        } else {
            shottypeMultiplier = (Rubrics.SURV[game].multiplier[shottype]
                ? Rubrics.SURV[game].multiplier[shottype]
                : 1)
        }
        points = (isPhantasmagoria(game)
            ? phantasmagoria(rubric, game, difficulty, shottypeMultiplier)
            : survivalPoints(rubric, game, difficulty, shottypeMultiplier))
    } else {
...
function phantasmagoria(rubric, game, difficulty, shottypeMultiplier) {
    var roundsLost = Number($(MISSES).val()),
        bonus;
    ...
    if (game == "IN") {
        if (difficulty == "Extra") {
            ...
            if (lastSpells > Rubrics.MAX_LAST_SPELLS[difficulty][route]) {
                if (language == "English") {
                    $(ERROR).html("<strong style='color:red'>Error: the number of Last Spells captured in a " + route + " clear on " + difficulty + " cannot exceed " + Rubrics.MAX_LAST_SPELLS[difficulty][route] + "</strong>")
                } else if (language == "Japanese") {
                    $(ERROR).html("<strong style='color:red'>エラー: ラストスペルが" + Rubrics.MAX_LAST_SPELLS[difficulty][route] + "を超えてはいけません。</strong>")
                } else {
                    $(ERROR).html("<strong style='color:red'>错误：" + route + "路线" + difficulty + "难度中的LSC收取数不能超过" + Rubrics.MAX_LAST_SPELLS[difficulty][route] + "。")
                }
                return 0
            }
            drcpoints += lastSpells * (difficulty == "Easy"
                ? 1
                : 2)
        }
    }
...
function mofFormula(difficulty, shottype) {
    ...
    if (score < thresholds.score[0]) {
        return Math.round(Math.pow((score / thresholds.score[0]), 2) * (difficulty == "Easy"
            ? 220
            : 200))
    }
    ...
    return Math.min(Math.round(drcpoints), (difficulty == "Easy"
        ? 375
        : 500))
}
function determineIncrement(thresholds, i) {
    ...
    var increment = (i === 1
            ? 20
            : 30),
        lowerBound = thresholds[i - 1] * 1000;
...
function scoringPoints(rubric, game, difficulty, shottype) {
    ...
    if (Rubrics.SCORE[game][difficulty].basedOn) {
        ...
        wrshottype = (game == "HSiFS" && difficulty != "Easy"
            ? removeSeason(shottype) + bestSeason(difficulty, shottype)
            : shottype);
        wr = WRs[game][difficulty][wrshottype][0]
    }
    exp = (game == "DDC" && difficulty == "Extra" && shottype == "MarisaB"
        ? 7
        : rubric.exp);
    return (score >= wr
        ? rubric.base
        : Math.round(rubric.base * Math.pow((score / wr), exp)))
}
function manoku(string, length, offset) {
    var original = string;
    for (index = length - 1; index > length - (offset + 1); index -= 1) {
        if (string.charAt(index) == '0') {
            string = string.removeAt(index)
        }
    }
    if (original.charAt(length - offset) != '0') {
        string = string.insertAt(length - offset, '.')
    }
...
function illion(string, length, offset) {
    var original = string;
    for (index = length - 1; index > length - (offset + 1); index -= 1) {
        if (string.charAt(index) == '0') {
            string = string.removeAt(index)
...
function abbreviate(num) {
    ...
    if (language != "English") {
        if (!string.contains('0') || string.length <= 4) {
            return string
        } else {
            return manoku(string, string.length, string.length <= 8
                ? 4
                : 8)
        }
    } else {
        if (string.length <= 6) {
            ...
            return illion(string, string.length, string.length <= 9
                ? 6
                : 9)
        }
    }
...
function generateRubrics() {
    ...
    $(SCORING + "1").html(translate("Scoring"));
    $(SCORE_FORMULA).html(translate("||Max * (Score/WR)^Exp||"));
    $(SURVIVAL + "1").html(translate("Survival"));
    $(SURV_FORMULA).html(translate("||Max * (Base^-n)||"));
    $(FWR_TABLE).html("<tr><th>" + translate("Category") + "</th><th>WR</th></tr>");
    $(WR_DEF_TABLE).html("<tr><th>" + translate("Category") + "</th><th>" + translate("Shottype") + "</th></tr>");
    for (game in Rubrics.SCORE) {
        $(SCORING_TABLE).append("<tr>" + (game == "HRtP"
            ? SCORING_COLUMN
            : "<th></th><td></td><td></td>") + "</tr>");
        for (difficulty in Rubrics.SCORE[game]) {
            rubric = Rubrics.SCORE[game][difficulty];
            $(SCORING_TABLE).append("<tr><th>" + translateGameName(game + " ") + difficulty + "</th><td>" + rubric.base + "</td><td>" + rubric.exp + "</td></tr>");
            if (rubric.basedOn) {
                $(WR_DEF_TABLE).append("<tr><th>" + translateGameName(game + " ") + difficulty + "</th><td>" + translateCharName(rubric.basedOn) + "</td></tr>")
            } else if (rubric.wr) {
                if (typeof rubric.wr == "object") {
                    for (shottype in rubric.wr) {
                        $(FWR_TABLE).append("<tr><th>" + translateGameName(game + " ") + difficulty + " " + translateCharName(shottype) + "</th><td>" + abbreviate(rubric.wr[shottype]) + "</td></tr>")
                    }
                } else {
                    $(FWR_TABLE).append("<tr><th>" + translateGameName(game + " ") + difficulty + "</th><td>" + abbreviate(rubric.wr) + "</td></tr>")
                }
            }
        }
    }
    for (game in Rubrics.SURV) {
        if (isPhantasmagoria(game)) {
            $(PHANTASMAGORIA).append("<tr>");
            $(PHANTASMAGORIA).append(game == "PoDD"
                ? "<th id='game1'>Game</th><th id='maxPoints2'>Max points</th><th id='minPoints'>Mi" +
                        "n points</th><th id='nbBonus'>No Bomb bonus</th>"
                : "<th></th><td></td><td></td><td></td>");
            $(PHANTASMAGORIA).append("</tr>")
        } else {
            $(SURV_TABLE).append("<tr>");
            $(SURV_TABLE).append(game == "HRtP"
                ? SURV_COLUMN
                : "<th></th><td></td><td></td><td></td><td></td><td></td>");
            $(SURV_TABLE).append("</tr>")
        }
        for (difficulty in Rubrics.SURV[game]) {
            rubric = Rubrics.SURV[game][difficulty];
            if (difficulty == "multiplier" || difficulty == "seasonMultiplier") {
                $(SHOTTYPE_MULTIPLIERS).append("<tr>");
                $(SHOTTYPE_MULTIPLIERS).append(game == "HRtP"
                    ? "<th id='multipliedShottype'>Shottype</th><th id='multiplier'>Multiplier</th><"
                    : "<th></th><td></td>");
                $(SHOTTYPE_MULTIPLIERS).append("</tr>");
                for (shottype in rubric) {
                    $(SHOTTYPE_MULTIPLIERS).append("<tr><th>" + translateGameName(game + " ") + (language == "Japanese"
                        ? "の"
                        : "") + (language == "Chinese"
                        ? " "
                        : "") + translateCharName(shottype) + "</th><td>" + rubric[shottype] + "</td></tr>")
                }
                continue
            }
            if (isPhantasmagoria(game)) {
                $(PHANTASMAGORIA).append("<tr><th>" + translateGameName(game + " ") + difficulty + "</th><td>" + rubric.base + "</td><td>" + rubric.min + "</td><td>" + rubric.noBombBonus + "</td></tr>")
            } else {
                $(SURV_TABLE).append("<tr><th>" + translateGameName(game + " ") + difficulty + "</th><td>" + rubric.base + "</td><td>" + rubric.exp + "</td><td>" + rubric.miss + "</td><td>" + rubric.firstBomb + "</td><td>" + rubric.bomb + "</td></tr>")
            }
        }
    }
    for (difficulty in Rubrics.MOF_THRESHOLDS) {
        ...
        for (shottype in Rubrics.MOF_THRESHOLDS[difficulty]) {
            ...
            thresholds = Rubrics.MOF_THRESHOLDS[difficulty][shottype];
            id += 1;
            for (i = 0; i < thresholds.base.length; i += 1) {
                if (language == "English") {
                    ...
                    $(MOF_TABLE).append("<tr><td>" + abbreviate(thresholds.score[i]) + "</td><td>" + thresholds.base[i] + "</td><td>" + abbreviate(thresholds.step[i]) + "ごとに+" + thresholds.increment[i] + "</td></tr>")
                } else {
                    $(MOF_TABLE).append("<tr><td>" + abbreviate(thresholds.score[i]) + "</td><td>" + thresholds.base[i] + "</td><td>每" + abbreviate(thresholds.step[i]) + "增加" + thresholds.increment[i] + "</td></tr>")
                }
            }
        }
    }
    $(DS_TABLE).append("<tr><th>" + translate("Scene") + "</th><th id='basePoints" + (id + 1) + "'>" + translate("Base points") + "</th><th id='increments" + (id) + "'>" + translate("Increments") + "</th><th id='threshold" + (id) + "'>" + translate("Threshold 1") + "</th><th id='increments" + (id + 1) + "'>" + translate("Increments") + "</th><th id='threshold" + (id + 1) + "'>" + translate("Threshold 2") + "</th><th id='increments" + (id + 2) + "'>" + translate("Increments") + "</th><th id='threshold" + (id + 2) + "'>" + translate("Threshold 3") + "</th></tr>");
    for (scene in Rubrics.SCENE_THRESHOLDS) {
        ...
        if (language == "English") {
            $(DS_TABLE).append("<tr><td>" + scene + "</td><td>0</td><td>+1 for every " + sep(step1) + "</td><td>" + sep(n1) + "</td><td>+1 for every " + sep(step2) + "</td><td>" + sep(n2) + "</td><td>+1 for every " + sep(step3) + "</td><td>" + sep(n3) + "</td></tr>")
        } else if (language == "Japanese") {
            $(DS_TABLE).append("<tr><td>" + scene + "</td><td>0</td><td>" + abbreviate(step1) + "ごとに+1</td><td>" + abbreviate(n1) + "</td><td>" + abbreviate(step2) + "ごとに+1</td><td>" + abbreviate(n2) + "</td><td>" + abbreviate(step3) + "ごとに+1</td><td>" + abbreviate(n3) + "</td></tr>")
        } else {
            $(DS_TABLE).append("<tr><td>" + scene + "</td><td>0</td><td>每" + abbreviate(step1) + "增加1</td><td>" + abbreviate(n1) + "</td><td>每" + abbreviate(step2) + "增加1</td><td>" + abbreviate(n2) + "</td><td>每" + abbreviate(step3) + "增加1</td><td>" + abbreviate(n3) + "</td></tr>")
        }
    }
}
function showRubrics(challenge) {
    generateRubrics();
    generateText(NOT_FIRST_TIME);
    $(challenge == "Survival"
        ? SURV_RUBRICS
        : SCORE_RUBRICS).css("display", "block");
    $(challenge == "Survival"
        ? SURV_BUTTON
        : SCORE_BUTTON).attr("onClick", "hideRubrics('" + challenge + "')");
    $(challenge == "Survival"
        ? SURV_BUTTON
        : SCORE_BUTTON).val(translate("Hide " + challenge + " Rubrics"))
}
function hideRubrics(challenge) {
    $(challenge == "Survival"
        ? SURV_RUBRICS
        : SCORE_RUBRICS).css("display", "none");
    $(challenge == "Survival"
        ? SURV_BUTTON
        : SCORE_BUTTON).attr("onClick", "showRubrics('" + challenge + "')");
    $(challenge == "Survival"
        ? SURV_BUTTON
        : SCORE_BUTTON).val(translate("Show " + challenge + " Rubrics"))
}
