<?php include_once 'assets/drc/drc_code.php'; setlocale(LC_ALL, $locale); bindtextdomain($lang, 'locale'); textdomain($lang) ?>
			...
			<?php
				foreach ($Rubrics['SCORE'] as $game => $value) {
					...
					foreach ($Rubrics['SCORE'][$game] as $diff => $rubric) {
						echo '<tr><th>' . _($game) . ($lang == 'en_US' ? ' ' : '') . $diff .
					...
					foreach ($Rubrics['SCORE'][$game] as $diff => $rubric) {
						if (!empty($rubric['wr']) && is_array($rubric['wr'])) {
		                    foreach ($rubric['wr'] as $shot => $score) {
		                        echo '<tr><th>' . _($game) . ($lang == 'en_US' ? ' ' : '') . $diff . ' ' . _($shot) .
			...
			<?php
				foreach ($Rubrics['SURV'] as $game => $value) {
					...
					foreach ($value as $diff => $rubric) {
			        	...
			        	echo '<tr><th>' . _($game) . ($lang == 'en_US' ? ' ' : '') . $diff . '</th><td>' . $rubric['base'] .
					...
					foreach ($value as $diff => $rubric) {
						if ($diff == 'multiplier' || $diff == 'seasonMultiplier') {
							foreach ($rubric as $shot => $mult) {
								echo '<tr><th>' . _($game) . ($lang == 'en_US' ? ' ' : '') . ($lang == 'ja_JP' ? 'の' : '') .
