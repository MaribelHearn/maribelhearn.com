var categories,
    gameCategories,
    defaultWidth = (navigator.userAgent.indexOf("Mobile") > -1 || navigator.userAgent.indexOf("Tablet") > -1)
        ? 60
        : 120,
    settings = {
        "categories": {
    ...
    maleCharacters = [
        "SinGyokuM", "Genjii", "Unzan", "RinnosukeMorichika", "FortuneTeller"
    ],
    pc98 = [
...
var reloadTiers = function () {
    ...
    for (tierNum in tierList) {
        ...
        if (!tierList[tierNum].flag) {
            $("#tier_list_tbody").append("<tr id='tr" + tierNum + "' onDragOver='allowDrop(event)' onDrop='drop(event)'><th id='th" + tierNum + "' class='tier_header' onClick='detectLeftCtrlCombo(event, " + tierNum + ")' onContextMenu='detectRightCtrlCombo(event, " + tierNum + "); return false;' style='color: " + tierList[tierNum].colour + "; background-color: " + tierList[tierNum].bg + ";'>" + tierList[tierNum].name + "</th><td id='tier" + tierNum + "' class='tier_content'></td></tr>");
            for (i = 0; i < tierList[tierNum].chars.length; i += 1) {
...
var initialise = function () {
    ...
    addTier("A");
    tmp = settings.sort;
    settings.sort = (settings.sort == "characters"
        ? "works"
        : "characters");
    addTier("S", noDisplay);
    addTier("A", noDisplay);
...
var addToTier = function (character, tierNum, pos, noDisplay) {
    var cats = (settings.sort == "characters"
        ...
        tierList = (settings.sort == "characters"
            ? tiers
            : gameTiers),
        categoryName = getCategoryOf(character),
        id,
        i;
...
var addMenu = function (character) {
    ...
    for (i = 0; i < tierOrder.length; i += 1) {
        tierNum = tierOrder[i];
        if (!tierList[tierNum].flag) {
            $("#mobile_modal").append("<input type='button' value='" + tierList[tierNum].name + "' onClick='addToTierMobile(\"" + character + "\", " + tierNum + ")' style='min-width: 25px; height: 25px; margin: 10px'>")
        }
    }
...
var removeFromTier = function (character, tierNum) {
    ...
    if (isMobile()) {
        $("#msg_container").html("<strong style='color:green'>Removed " + $("#" + character).attr("alt") + " from " + tierList[tierNum].name + "!</strong>")
    }
    window.onbeforeunload = function () {
        return confirm()
    }
...
var changeToTier = function (character, tierNum) {
    var oldTierNum = getTierNumOf(character),
        help,
        id;
...
var modalChar = function (character, tierNum) {
    var tierOrder = (settings.sort == "characters"
            ? order
            : gameOrder),
        above,
        below;
    emptyModal();
    $("#mobile_modal").html("<h3>" + character + "</h3><input type='button' value='Remove' onClick='removeFromTier(\"" + character + "\", " + tierNum + "); emptyModal()' style='margin:10px'>");
    if (tierOrder.indexOf(tierNum) !== 0) {
        above = tierOrder[tierOrder.indexOf(tierNum) - 1];
        $("#mobile_modal").append("<input type='button' value='Move Up' onClick='changeToTier(\"" + character + "\", " + above + "); emptyModal()' style='margin:10px'>")
    }
    if (tierOrder.indexOf(tierNum) != order.length - 1) {
        below = tierOrder[tierOrder.indexOf(tierNum) + 1];
        $("#mobile_modal").append("<input type='button' value='Move Down' onClick='changeToTier(\"" + character + "\", " + below + "); emptyModal()' style='margin:10px'>")
    }
    $("#mobile_modal").append("<input type='button' value='Move to Back' onClick='moveToBack(\"" + character + "\", " + tierNum + "); emptyModal()' style='margin:10px'>");
    $("#mobile_modal").css("display", "block");
    $("#modal").css("display", "block")
};
...
var addTier = function (tierName, noDisplay) {
    ...
    if (!noDisplay) {
        $("#tier_list_tbody").append("<tr id='tr" + tierNum + "' onDragOver='allowDrop(event)' onDrop='drop(event)'><th id='th" + tierNum + "' class='tier_header' onClick='detectLeftCtrlCombo(event, " + tierNum + ")' onContextMenu='detectRightCtrlCombo(event, " + tierNum + "); return false;' style='max-width:" + settings.tierHeaderWidth + "px" + (isMobile()
            ? ";height:60px"
            : "") + "'>" + tierName + "</th><td id='tier" + tierNum + "' class='tier_content'></td></tr><hr>")
    }
    tierList[tierNum] = {};
...
var startTierSwap = function (tierNum) {
    ...
    var tierName = $("#th" + tierNum).html(),
        invertedColour = new RGBColor(tierList[tierNum].colour),
        invertedBg = new RGBColor(tierList[tierNum].bg),
        otherTierNum;
...
var swapTiers = function (tierNum1, tierNum2) {
    var tierList = (settings.sort == "characters"
        ? tiers
        : gameTiers);
    var tmp = tierList[tierNum1],
        tierNum;
    $("#th" + tierNum1).attr("style", "color: " + tierList[tierNum2].colour + "; background-color: " + tierList[tierNum2].bg + "; max-width: " + settings.tierHeaderWidth + "px; width: " + settings.tierHeaderWidth + "px; height: " + tierHeaderHeight() + "px;");
    $("#th" + tierNum2).attr("style", "color: " + tierList[tierNum1].colour + "; background-color: " + tierList[tierNum1].bg + "; max-width: " + settings.tierHeaderWidth + "px; width: " + settings.tierHeaderWidth + "px; height: " + tierHeaderHeight() + "px;");
...
var removeTier = function (tierNum, skipConfirmation) {
    var tierList = (settings.sort == "characters"
        ...
        tierOrder = (settings.sort == "characters"
            ? order
            : gameOrder);
    var length = tierList[tierNum].chars.length,
        confirmation = true,
        otherTierNum,
        i;
    if (isMobile()) {
    ...
    if (!skipConfirmation) {
        confirmation = confirm("Are you sure you want to remove this tier? Removal may take a moment with many c" +
                "haracters inside.")
    }
    if (confirmation) {
...
var toggleInstructions = function () {
    $(".instructions").css("display", $(".instructions").css("display") == "none"
        ? "block"
        : "none");
    $("#toggle").html("<a href='javascript:toggleInstructions()'>Click here to " + ($(".instructions").css("display") == "none"
        ? "show"
        : "hide") + " the instructions.</a>")
};
var allowData = function () {
    if (localStorage.length <= 2) {
        return confirm("This will store data in your browser's Web Storage, which functions like a cooki" +
                "e. Do you allow this?")
    } else {
        return true
    }
};
var saveTiersData = function () {
    if (isMobile()) {
        emptyModal()
    }
...
var saveTiers = function () {
    if (isMobile() && localStorage.length <= 2) {
        emptyModal();
        $("#mobile_modal").html("<h3>Save Tiers</h3><p>This will store data in your browser's Web Storage, which " +
                "functions like a cookie. Do you allow this?</p>");
        $("#mobile_modal").append("<input type='button' value='Yes' onClick='saveTiersData()' style='margin: 10px'>");
...
var saveSettingsPre = function () {
    if (isMobile() && localStorage.length <= 2) {
        emptyModal();
        $("#mobile_modal").html("<h3>Save Settings</h3><p>This will store data in your browser's Web Storage, whi" +
                "ch functions like a cookie. Do you allow this?</p>");
        $("#mobile_modal").append("<input type='button' value='Yes' onClick='saveSettingsData()' style='margin: 10p" +
                "x'>");
        $("#mobile_modal").append("<input type='button' value='No' onClick='emptyModal()' style='margin: 10px'>");
        $("#mobile_modal").css("display", "block");
        $("#modal").css("display", "block");
...
var modalInformation = function () {
    emptyModal();
    $("#mobile_modal").html("<h3>Acknowledgements</h3>" + $("#credits").html() + "<h3>Instructions</h3>" + $("#instructions").html());
    $("#mobile_modal").css("display", "block");
    $("#modal").css("display", "block")
};
var menu = function () {
    emptyModal();
    $("#mobile_modal").html("<h3>Menu</h3>" + $("#menu").html() + "<h3>Navigation</h3><a href='/'><img src='favicon.ico' alt='Index icon'> Index</a" +
...
var importText = function () {
    var tierNum,
        ...
        i;
    emptyModal();
    $("#text_conversion").html("<h2>Import from Text</h2><p>Note that the format should be the same as the expor" +
            "ted text.</p>");
    $("#text_conversion").append("<p><strong>Warning:</strong> Importing will overwrite your current tier list!");
    $("#text_conversion").append("<textarea id='import'></textarea><p><input type='button' value='Import' onClick=" +
            "'load()'></p>");
...
var copyToClipboard = function () {
    navigator
        .clipboard
        .writeText($("#text").html().replace(/<\/p><p>/g, "\n").strip());
    emptyModal();
...
var exportText = function () {
    ...
    $("#text_conversion").append("<p><input type='button' value='Copy to Clipboard' onClick='copyToClipboard()'></" +
            "p>");
    for (i = 0; i < tierOrder.length; i += 1) {
        tierNum = tierOrder[i];
        if (!tierList[tierNum].flag) {
...
var customiseMenu = function () {
    ...
    $("#customisation").append("</div><div><p><input type='button' value='Save Changes' onClick='saveCustom()'><" +
...
var settingsMenuChars = function () {
    var categoryName,
        ...
        counter = 0;
    emptyModal();
    $("#settings").html("<h2>Settings</h2><div>Use the following art set:<form id='artist_form'></form></" +
            "div>");
    ...
    $("#artist_form").append("<label for='ruu'>るう</label><input id='ruu' name='artist' type='radio'" + (settings.artist == "Ruu"
        ? " checked"
        : "") + ">");
    $("#settings").append("Include characters in the following works of first appearance:<table id='setting" +
    ...
    $("#settings").append("<p><label for='pc-98'>PC-98</label><input id='pc98' type='checkbox' onClick='tog" +
            "glePC98()'" + (settings.pc98Enabled
        ? " checked"
        : "") + " " + (settings.artist == "Ruu"
        ? "disabled=true"
        : "") + "></p>");
    $("#settings").append("<p><label for='windows'>Windows</label><input id='windows' type='checkbox' onCli" +
            "ck='toggleWindows()'" + (settings.windowsEnabled
        ? " checked"
        : "") + "></p>");
    $("#settings").append("<p><label for='male'>Male Characters</label><input id='male' type='checkbox' onC" +
    ...
    $("#settings").append("<div><p><input type='button' value='Save Changes' onClick='saveSettings()'></p><" +
...
var settingsMenuWorks = function () {
    var categoryName,
        ...
        counter = 0;
    emptyModal();
    $("#settings").html("<h2>Settings</h2>Include works in the following categories:<table id='settings_t" +
...
var togglePC98 = function () {
    for (var i = 0; i < pc98.length; i += 1) {
        $("#checkbox_" + pc98[i]).prop("checked", $("#pc98").is(":checked")
            ? true
            : false)
    }
};
var toggleWindows = function () {
    for (var i = 0; i < windows.length; i += 1) {
        $("#checkbox_" + windows[i]).prop("checked", $("#windows").is(":checked")
            ? true
            : false)
    }
};
var toggleMale = function () {
    $("#checkbox_Soku").prop("checked", $("#male").is(":checked")
        ? true
        : false)
};
...
var saveSettings = function () {
    ...
    if (removedCategories.length > 0) {
        confirmation = confirm("This will remove characters from disabled categories from the current tiers. Are" +
    ...
    if (settings.sort == "characters") {
        ...
        settings.artist = $("#dairi").is(":checked")
            ? "Dairi"
            : "Ruu";
    ...
    settings.tierHeaderWidth = $("#tierHeaderWidth").val() > defaultWidth
        ? $("#tierHeaderWidth").val()
        : defaultWidth;
...
var toggleTierView = function () {
    if (isMobile()) {
        tierView = !tierView;
        $("#tier_list_container").css("display", tierView
            ? "block"
            : "none");
        $("#characters").css("display", tierView
            ? "none"
            : "block");
        $("#hide").val(tierView
            ? "Picker View"
        ...
        $("#characters").css("display", tierView
            ? "block"
            : "none");
        $("#buttons").css("display", tierView
            ? "block"
            : "none");
        tierView = !tierView;
        $("#show").css("display", tierView
            ? "block"
            : "none");
        $("#wrap").css("max-height", tierView
            ? "100%"
            : "77%");
        $("#wrap").css("width", tierView
            ? "auto"
            : "48%");
        $("#wrap").css("bottom", tierView
            ? "5px"
            : "");
        $("#wrap").css("left", tierView
            ? "5px"
            : "")
    }
};
...
var eraseAll = function () {
    ...
    if (isMobile()) {
        emptyModal();
        $("#mobile_modal").html("<h3>Reset</h3><p>Are you sure you want to reset your tier list and settings to t" +
                "he defaults?</p>");
        $("#mobile_modal").append("<input type='button' value='Yes' onClick='eraseAllConfirmed(); emptyModal()' sty" +
                "le='margin: 10px'>");
        $("#mobile_modal").append("<input type='button' value='No' onClick='emptyModal()' style='margin: 10px'>");
        $("#mobile_modal").css("display", "block");
        $("#modal").css("display", "block");
...
var loadTier = function (tiersData, tierNum, tierSort) {
    var tierList = (tierSort == "characters"
            ? tiers
            : gameTiers),
        character;
    tierList[tierNum] = {};
    ...
    if (!tierList[tierNum].flag) {
        if (tierSort == settings.sort) {
            $("#tier_list_tbody").append("<tr id='tr" + tierNum + "' onDragOver='allowDrop(event)' onDrop='drop(event)'><th id='th" + tierNum + "' class='tier_header' onClick='detectLeftCtrlCombo(event, " + tierNum + ")' onContextMenu='detectRightCtrlCombo(event, " + tierNum + "); return false;' style='color: " + tierList[tierNum].colour + "; background-color: " + tierList[tierNum].bg + ";'>" + tiersData[tierNum].name + "</th><td id='tier" + tierNum + "' class='tier_content'></td></tr>");
...
var loadTiersFromStorage = function () {
...
var loadCharacters = function () {
    ...
    for (categoryName in categories) {
        ...
        for (i in categories[categoryName].chars) {
            ...
            if (isMobile()) {
                $("#" + categoryName).append("<span id='" + character.removeSpaces() + "C'><img id='" + character.removeSpaces() + "' class='list' onClick='addMenu(\"" + character + "\")' src='art/" + settings.artist + "/" + categoryName + "/" + character.removeSpaces() + "." + (settings.artist == "Dairi"
                    ? "png"
                    : "jpg") + "' alt='" + character + "'>")
            } else {
                $("#" + categoryName).append("<span id='" + character.removeSpaces() + "C'><img id='" + character.removeSpaces() + "' class='list'  onDblClick='addToMostRecent(\"" + character.removeSpaces() + "\")' draggable='true' onDragStart='drag(event)' src='art/" + settings.artist + "/" + categoryName + "/" + character.removeSpaces() + "." + (settings.artist == "Dairi"
                    ? "png"
...
var loadWorks = function () {
    ...
    for (categoryName in gameCategories) {
        ...
        for (i in gameCategories[categoryName].chars) {
            game = gameCategories[categoryName]
                .chars[i]
                .replace("'", "");
            if (isMobile()) {
                $("#" + categoryName).append("<span id='" + game.removeSpaces() + "C'><img id='" + game.removeSpaces() + "' class='list' onClick='addMenu(\"" + game + "\")' src='games/" + acronym(game) + "120x120.jpg' alt='" + game + "'>")
            } else {
                $("#" + categoryName).append("<span id='" + game.removeSpaces() + "C'><img id='" + game.removeSpaces() + "' class='list'  onDblClick='addToMostRecent(\"" + game.removeSpaces() + "\")' draggable='true' onDragStart='drag(event)' src='games/" + acronym(game) + "120x120.jpg' alt='" + game + "' title='" + game + "'>")
            }
        }
        $("#characters").append("</div>");
...
var loadSettingsFromStorage = function () {
    ...
    if (settingsData.hasOwnProperty("categories")) {
        ...
        settings.tierHeaderWidth = (settingsData.tierHeaderWidth
            ? settingsData.tierHeaderWidth
            : defaultWidth);
...
var applyMobileCSS = function () {
    ...
    $("#instructions_list").html("<li>Tap a character to add them to a tier.</li><li>Long press a character in a t" +
            "ier to either remove them from that tier, move them to the back of the tier, or " +
            "move them a tier up or down.</li><li>Tap a tier and then another tier to swap th" +
            "eir positions.</li><li>Long press a tier to either remove it, remove all its cha" +
            "racters, or add all remaining characters to it.</li>");
    ...
    $("#buttons").css("height", isLandscape()
        ? "22%"
        : "16%");
    $("#buttons").css("min-height", "50px");
    $("#characters").css("width", "auto");
    $("#characters").css("top", isLandscape()
        ? "2%"
        : "50px");
    $("#characters").css("right", "5px");
    $("#characters").css("bottom", isLandscape()
        ? "25%"
        : "18%");
    ...
    $("#modal").css("padding-top", isLandscape()
        ? "2%"
        : "10%")
};
