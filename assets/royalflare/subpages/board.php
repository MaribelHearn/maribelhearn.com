    ...
    foreach ($diffs as $key => $diff) {
        ...
        if ($game == 'GFW' && $diff == 'Extra') {
            ...
            foreach ($shots as $key => $shot) {
                ...
                '<th>難易度<br>Difficulty</th><th>プレイ日付<br>Play Date</th><th>名前<br>Player</th><th>コメント<br>Comment</th><th>リプレイ<br>Replay</th></tr>';
                foreach ($board as $key => $entry) {
                    $slowdown_class = (check_slowdown($game, $entry['slowdown']) ? ' class="slowdown"' : '');
                    if ($entry['difficulty'] == $diff && (($game != 'GFW' && $entry['chara'] == $shot) || ($game == 'GFW' && $entry['route'] == $shot)) && ($game != 'IN' || $entry['route'] == 'Final' . $route)) {
                        ...
                        '<td>' . $entry['difficulty'] . ($game == 'IN' ? '<br>' . (tl_term('Final' . $route, $lang_code)) : '') . '</td><td>' . $entry['date'] . '</td><td>' . $entry['player'] .
