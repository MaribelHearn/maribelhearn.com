	...
	<?php
        ...
        if (!empty($subpage)) {
            if (strpos($subpage, '/') !== false) {
                $tmp = preg_split('/\//', $subpage);
                if (($tmp[0] == 'history' || $tmp[0] == 'standings') && !empty(game_to_abbr($tmp[1]))) {
                    include_once 'assets/royalflare/' . $tmp[0] . '/' . $tmp[1] . '.' . ($tmp[0] == 'standings' ? 'html' : 'php');
                ...
                if ($subpage == 'search') {
                    ...
                    include_once 'assets/royalflare/subpages/board_th125.php';
                } else if ($subpage == 'th095' || $subpage == 'th125' || $subpage == 'th143' || $subpage == 'th165') {
                    include_once 'assets/royalflare/subpages/board_scene.php';
                } else {
                    if (!empty(game_to_abbr($subpage))) {
                        include_once 'assets/royalflare/subpages/board.php';
                    } else if (file_exists('assets/royalflare/subpages/' . $subpage . '.php')) {
                        include_once 'assets/royalflare/subpages/' . $subpage . '.php';
                    } else {
                        $exists = false;
                        echo '<p>No such page.</p>';
                    }
