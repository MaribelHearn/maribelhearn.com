...
function generateText() {
    if (language == "English") {
        ...
        $("#lastupdate").html(translateDate(lastUpdate, "YMD") + "現在のLNN記録です。");
        ...
        $("#lastupdate").html("LNN更新于" + translateDate(lastUpdate, "YMD") + "。");
        $("#contents_header").html("内容");
        $("#lnns").html("LNN列表");
...
function show(game) {
    ...
    $("#listhead").html("<tr><th class='shottype'>Shottype</th><th class='sorttable_numeric'><span id='nu" +
            "meric' class='nooflnn" + (restrictions(game)
        ? restrictions(game)
...
function load() {
    $
        .get("https://maribelhearn.com/json/lnnlist.json", function (data) {
            LNNs = data;
            lastUpdate = LNNs.LM;
            var playergameLNNs = {},
                playerLNNs = {},
            ...
            for (player in playerLNNs) {
                $("#ranking_tbody").append("<tr><td>" + player + "</td><td>" + playerLNNs[player] + "</td><td>" + (playergameLNNs[player].length == 12
                    ? playergameLNNs[player].length + "<span class='all'> (All)</span>"
                    : playergameLNNs[player].length) + "</td></tr>")
            }
            $("#on").html(count);
