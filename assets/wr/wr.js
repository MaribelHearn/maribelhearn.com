...
function toggleDates(alreadyDisabled) {
    ...
    $(".date, .datestring, .datestring_game").css("display", datesEnabled
        ? "inline"
        : "none")
}
...
function load() {
    ...
    if (notation != "DMY") {
        ...
        //$("#lm").html(translateDate($("#lm").html(), notation));
...
function generateDates(oldLanguage, oldNotation) {
    ...
    if (selected !== "") {
        ...
        if (!oldNotation) { // game opened, WR table DMY -> MDY/YMD
            for (i = 0; i < datestrings.length; i++) {
                $(datestrings[i]).html(translateDate($(datestrings[i]).html(), notation))
            }
        } else if (notation != oldNotation) { // language changed
            for (i = 0; i < datestrings.length; i++) {
                if (oldNotation == "DMY") { // WR table DMY -> MDY/YMD
                    $(datestrings[i]).html(translateDate($(datestrings[i]).html(), notation))
                } else if (oldNotation == "MDY") { // WR table MDY -> DMY/YMD
                    $(datestrings[i]).html(translateUSDate($(datestrings[i]).html(), notation))
                } else if (oldNotation == "YMD") { // WR table YMD -> DMY/MDY
