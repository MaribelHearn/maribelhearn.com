...
function generateText() {
    ...
    if (language == "English") {
        ...
        $("#lastupdate").html("World records are current as of " + (notation == "MDY"
            ? translateDate(lastUpdate, "MDY")
            : lastUpdate) + ".");
        ...
        $("#lastupdate").html("世界记录更新于" + translateDate(lastUpdate, "YMD") + ".");
        $("#contents_header").html("内容");
...
function display(game, seasonSwitch) {
    ...
    var shottypes = shottypeArray(game),
        compareWRs = {},
        max = 0,
        game,
        difficulty,
    ...
    $("#list_thead").html("<tr id='list_thead_tr'><th class='shottype'>Shottype</th></tr>");
    $("#list_tbody").html("");
    ...
    for (difficulty in WRs[game]) {
        if (game == "GFW" && difficulty == "Extra") {
            $("#list_tbody").append("<tr id='" + shottypes[i] + "'><td>Extra</td><td id='GFWExtra-' colspan='4'></td></tr>")
            ...
            for (i = 0; i < 4; i += 1) {
                $("#" + extrashots[i] + "Spring").append("<td id='" + game + difficulty + extrashots[i] + "' rowspan='4'></td>")
            }
        } else {
        ...
        for (shottype in WRs[game][difficulty]) {
            ...
            if (wr[3]) {
                ...
                replay = (gameAbbr(game) < 6
                    ? ""
                    : replayPath(game, difficulty, shottype))
            }
            if (score > max) {
            ...
            if (game == "HSiFS" && shottype.removeChar() == bestSeason(difficulty, seasonless)) {
                $("#" + game + difficulty + seasonless + (difficulty == "Extra"
                    ? "Small"
                    : "")).html(text + (game == "HSiFS" && difficulty != "Extra"
    ...
    $("#" + game + "overall4").html("<span class='datestring'>" + (overalldate
        ? overalldate
        : '-') + "</span>");
    $("#west_tbody").html("");
    if (!datesEnabled) {
...
function checkWindows() {
    ...
    for (var key in windows) {
        if (checked) {
            $("#" + windows[key] + "c").prop("checked", true);
            show(windows[key], false);
            skips.remove(windows[key])
        } else {
            $("#" + windows[key] + "c").prop("checked", false);
...
function checkPC98() {
    ...
    for (var key in pc98) {
        if (checked) {
            $("#" + pc98[key] + "c").prop("checked", true);
            show(pc98[key], false);
            skips.remove(pc98[key])
        } else {
            $("#" + pc98[key] + "c").prop("checked", false);
            hide(pc98[key], false);
...
function checkAll() {
    ...
    for (var key in all) {
        if (checked) {
            $("#" + all[key] + "c").prop("checked", true);
            show(all[key], false);
            skips.remove(all[key])
        } else {
            $("#" + all[key] + "c").prop("checked", false);
            hide(all[key], false);
...
function toggleDates() {
    ...
    for (i in all) {
        ...
        $("#" + all[i] + "overall4").css("display", datesEnabled
            ? "table-cell"
            : "none")
    }
    $(".date").css("display", datesEnabled
        ? "table-cell"
        : "none");
    $(".datestring").css("display", datesEnabled
        ? "inline"
        : "none")
}
...
function load() {
    $
        .get("https://maribelhearn.com/json/wrlist.json", function (data) {
            ...
            for (game in WRs) {
                ...
                for (difficulty in WRs[game]) {
                    for (shottype in WRs[game][difficulty]) {
                        ...
                        month = Number(tmp[1]);
                        day = Number(tmp[0]);
                        tmp = lastUpdate.split('/');
                        if (year > Number(tmp[2])) {
                            lastUpdate = date
                        } else if (year == Number(tmp[2]) && month > Number(tmp[1])) {
                            lastUpdate = date
                        } else if (year == Number(tmp[2]) && month == Number(tmp[1]) && day > Number(tmp[0])) {
                            lastUpdate = date
                        }
                        if (score > max) {
                            overall = "#" + game + difficulty + shottype;
            ...
            for (player in playerWRs) {
                ...
                $("#ranking_tbody").append("<tr id='player_" + player + "'><td>" + player + "</td><td id='player_" + player + "n'>" + count + "</td><td id='player_" + player + "g'>" + Object.keys(playerWRs[player]).length + "</td></tr>")
            }
            if (getCookie("lang") == "Japanese") {
                ...
                language = "English";
                notation = (getCookie("datenotation")
                    ? getCookie("datenotation")
                    : "DMY");
                if (notation == "DMY") {
                    $("#lastupdate").html("World records are current as of " + lastUpdate + ".")
                } else if (notation == "MDY") {
                    $("#lastupdate").html("World records are current as of " + translateDate(lastUpdate, "MDY") + ".");
                    var datestrings = $(".datestring");
    ...
    $.get("https://maribelhearn.com/json/bestinthewest.json", function (data) {
        westScores = data
    }, "json")
}
